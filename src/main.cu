...
/* First order derivatives */
	...
	int *d_fod;
	float *d_fod_exact_dirs;
	unsigned char *d_fod_image, *d_fod_discrete_dirs, *d_fod_dirs_image;

	CUDA_CHECK(cudaMalloc(&d_fod, size * 2 * sizeof(int)));
	CUDA_CHECK(cudaMalloc(&d_fod_exact_dirs, size * sizeof(float)));
	CUDA_CHECK(cudaMalloc(&d_fod_image, size * 3));
	CUDA_CHECK(cudaMalloc(&d_fod_discrete_dirs, size));
	CUDA_CHECK(cudaMalloc(&d_fod_dirs_image, size));

	// Launch kernel
	fod<<<blocks, threads>>>(d_input_image, d_fod, d_fod_image, d_fod_exact_dirs, d_fod_discrete_dirs, d_fod_dirs_image, width, height);
...
/* Second order derivatives and watershed */
	...
	// Launch kernel
	sod_and_watershed<<<blocks, threads>>>(d_fod, d_sod_image, d_watershed,
																				 width, height);
	CUDA_CHECK(cudaDeviceSynchronize());
...
/* Non maximum suppression */
	...
	// Launch kernel
	non_maximum_suppression<<<blocks, threads>>>(d_input_image, d_fod_discrete_dirs,
...
/* Relaxed cone map generation: Baseline */
	...
	// Launch kernel
	create_cone_map_baseline<<<blocks, threads>>>(d_input_image, d_fod_image, d_fod_exact_dirs, d_watershed, d_cone_map,
			width, height);
	CUDA_CHECK(cudaDeviceSynchronize());

	// Write result image to file
	write_device_texture_to_file((output_name + "_relaxed_cone_map_baseline.png").c_str(),
...
/* Relaxed cone map generation: Analytic */
	// Launch kernel
	create_cone_map_analytic<<<blocks, threads>>>(d_input_image, d_fod_image, d_fod_discrete_dirs, d_suppressed,
...
/* Cleanup */
	...
	CUDA_CHECK(cudaFree(d_fod_exact_dirs));
	CUDA_CHECK(cudaFree(d_fod_discrete_dirs));
